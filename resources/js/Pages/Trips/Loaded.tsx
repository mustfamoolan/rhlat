...
export default function LoadedTrips({ trips, totalSum, filters }: LoadedPageProps) {
    ...
    const handleFilter = () => {
        router.get(route('trips.loaded'), { search, date }, { preserveState: true });
    };

    const handleResetFilter = () => {
        setSearch('');
        setDate('');
        router.get(route('trips.loaded'), {}, { preserveState: true });
    };
    ...
    return (
        <MainLayout title="الرحلات المحملة">
            <Head title="الرحلات المحملة - رحلات" />

            <CreateModal open={createModalOpen} onOpenChange={setCreateModalOpen} defaultType="loaded" />
